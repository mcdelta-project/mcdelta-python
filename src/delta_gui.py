...
def gui_setup_config(_instance):
    ...
    instance = _instance
    modfolder, jarfolder, mc_version = read_config(
        _instance)  # gets config stuff
    os.chdir(execdir)
    # transferring config data (and Tkinter instance) to all files
    init_config_util(
        (modfolder, jarfolder, mc_version, execdir, instance, gui))
    delta_install.init_config_install(
        (modfolder, jarfolder, mc_version, execdir, instance, gui))
    delta_remove.init_config_remove(
        (modfolder, jarfolder, mc_version, execdir, instance, gui))
    delta_upgrade.init_config_upgrade(
        (modfolder, jarfolder, mc_version, execdir, instance, gui))
...
def sdinst():
    ...
    if(name == read_default_instance()):
        msgbox.showerror("Instance already default", "Instance " +
                         name+" is already set as default.", parent=tkinst)
    else:
        with open(execdir+"/LocalData/default_instance.txt", "w") as f:
            f.write(name)
        msgbox.showinfo("Default instance set",
                        "Set default instance as "+name+".", parent=tkinst)


def addinst():
    name = dialogs.askstring(
        "Instance Name", "Enter name for new instance:", parent=tkinst)
    if(name == None):
        return -1
    if(instance_exists(name)):
        msgbox.showerror("Instance already exists", "Instance " +
                         name+" already exists.", parent=tkinst)
    else:
        new_config(name)
        tkinst.ilist["menu"].add_command(
            label=name, command=lambda n=name: tkinst.isel.set(n))
        msgbox.showinfo("Instance created", "Instance " +
                        name+" created.", parent=tkinst)


def removinst():
    name = tkinst.isel.get()
    if(not instance_exists(name)):
        msgbox.showerror("Instance does not exist", "Instance " +
                         name+" does not exist. Cannot remove.", parent=tkinst)
    elif(instance == name):
        msgbox.showerror("Instance currently selected", "Instance " +
                         name+" is currently selected. Cannot remove.", parent=tkinst)
    else:
        rm_config(name)
        msgbox.showinfo("Instance removed", "Instance " +
                        name+" removed.", parent=tkinst)

    tkinst.ilist["menu"].delete(0, tk.END)
    insts = list(get_all_insts())
    for inst in insts:
        tkinst.ilist["menu"].add_command(
            label=inst, command=lambda n=inst: tkinst.isel.set(n))
...
class Gui(tk.Frame):
    ...
    def initialise_window(self):
        ...
        self.winv = tk.PanedWindow(
            self, orient=tk.VERTICAL, sashrelief=tk.RAISED, height=400, width=800)
        self.winv.pack(expand=True, fill=tk.BOTH)

        self.win = tk.PanedWindow(
            self, orient=tk.HORIZONTAL, sashrelief=tk.RAISED, height=300, width=800)
        self.winv.add(self.win)

        self.winl = tk.PanedWindow(
            self, orient=tk.HORIZONTAL, sashrelief=tk.RAISED, height=100, width=800)
        ...
        self.console = tk.Text(self.cpane, height=4,
                               yscrollcommand=self.consoles.set)
        ...
        self.instmod = tk.Button(
            self.bpane, text="Install Mods", command=instmods)
        self.instmod.pack()

        self.rmod = tk.Button(
            self.bpane, text="Remove Mods", command=removmods)
        self.rmod.pack()

        self.umod = tk.Button(
            self.bpane, text="Upgrade Mods", command=upgrmods)
        self.umod.pack()
        ...
        self.addinst = tk.Button(
            self.lpane, text="Add Instance...", command=addinst)
        self.addinst.pack()

        self.reminst = tk.Button(
            self.lpane, text="Remove Instance", command=removinst)
        self.reminst.pack()

        self.setinst = tk.Button(
            self.lpane, text="Select Instance", command=updateinst)
        self.setinst.pack()

        self.definst = tk.Button(
            self.lpane, text="Set as Default Instance", command=sdinst)
        self.definst.pack()
        ...
        self.explist = tk.Button(
            self.lpane, text="Export Mod List...", command=exportmlist)
        self.explist.pack(side=tk.BOTTOM)

        self.implist = tk.Button(
        ...
        self.mlist = tk.Listbox(
            self.mpane, selectmode=tk.MULTIPLE, yscrollcommand=self.mlists.set)
        ...
        self.mlisti = tk.Listbox(
            self.rpane, selectmode=tk.MULTIPLE, yscrollcommand=self.mlistsi.set)
    ...
    if(command.split(" ")[0] == "update"):
        ...
        if(len(command.split(" ")) >= 2):
            ...
            for item in modslist:
                string = string + item+", "
            # [:-2] to cut off the extra ", " after the last element
            cprint(string[:-2]+"...")
            for item in modslist:
