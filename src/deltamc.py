...
if __name__ == "__main__":
    ...
    valid_actions_with_args = ['install', 'remove',
                               'upgrade', 'search', 'export', 'import', 'instance']
    valid_actions_no_args = ['gui', 'help']

    args = {'gui': False, 'instance': "None", 'install': "None", 'remove': "None",
            'upgrade': "None", 'search': "None", 'export': "None", 'importa': "None"}
    # 1 argument
    if len(sys.argv) == 2 and sys.argv[1] in valid_actions_with_args:
    ...
    # 1 argument
        ...
        if action == 'help':
            ...
            args["gui"] = True
    # 2 arguments
    elif len(sys.argv) >= 3 and sys.argv[1] in valid_actions_with_args:
        action = sys.argv[1]
    ...
    try:
        with open("LocalData/config.json") as json_file:
            ...
            insts = json_data.keys()
        for inst in insts:
            # creating modsdownloaded subdirs for each instance
            if(not os.path.exists(os.path.join(execdir, "LocalData/ModsDownloaded/"+inst))):
    ...
    if (upgradesavailable == []):
        ...
        for upgrade in upgradesavailable:
            cprint(" "+upgrade[0].name+" (current version: "+upgrade[1].versions[0]
                   ["Version"]+", you have: "+upgrade[0].versions[0]["Version"]+")")
